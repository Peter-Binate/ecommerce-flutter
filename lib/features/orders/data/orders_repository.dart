...
class OrderRecord {
  ...
  const OrderRecord({required this.id, required this.total, required this.createdAt});
...
class OrdersRepository {
  ...
  Future<List<OrderRecord>> getOrders() async {
    ...
    return list.map((e) => OrderRecord.fromJson(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<void> addOrder(OrderRecord order) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_ordersKey, json.encode(updated.map((e) => e.toJson()).toList()));
  }
}
