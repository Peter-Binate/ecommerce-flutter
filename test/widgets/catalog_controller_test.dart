...
void main() {
  ...
  group('CatalogController Filtering Tests', () {
    test(
      'Le getter "filtered" doit retourner tous les produits si la recherche est vide',
      () async {
        // <-- 1. Ajouter async
    ...
    test(
      'Le getter "filtered" doit retourner les produits correspondants à la catégorie',
      () async {
        final mockRepo = MockCatalogRepository();
        when(mockRepo.fetchProducts()).thenAnswer((_) async => mockProducts);
        ...
        expect(
          controller.filtered.every((p) => p.category == 'electronics'),
          isTrue,
        );
      },
    );
    ...
    test(
      'Le getter "filtered" doit retourner les produits correspondants au titre',
      () async {
        final mockRepo = MockCatalogRepository();
        when(mockRepo.fetchProducts()).thenAnswer((_) async => mockProducts);
        final controller = CatalogController(mockRepo);
        ...
        expect(
          controller.filtered.any((p) => p.title == 'Souris Gamer Pro'),
          isTrue,
        );
      },
    );
    test(
      'Le getter "filtered" ne doit retourner aucun produit si la recherche ne correspond à rien',
      () async {
        final mockRepo = MockCatalogRepository();
        when(mockRepo.fetchProducts()).thenAnswer((_) async => mockProducts);
